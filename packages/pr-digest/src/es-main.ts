// From: https://github.com/tschaub/es-main/blob/main/main.js
...
export default function esMain(meta: ImportMeta) {
  ...
  // casts required below here, because ts narrows from the `main` in meta check, but main
  // isn't defined before node 22.18
  const require = createRequire((meta as ImportMeta).url);
  const scriptPath = require.resolve(process.argv[1]);

  const modulePath = fileURLToPath((meta as ImportMeta).url);
